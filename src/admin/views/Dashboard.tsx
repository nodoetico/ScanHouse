      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-display text-[var(--sh-text)] text-lg">Propiedades con mayor interacción</h3>
            <span className="font-mono text-[10px] uppercase tracking-widest text-[var(--sh-faint)]">Últimos 30 días</span>
          </div>
              ...
              <tbody>
                {top.map(p => (
                  <tr key={p.id} className="border-t border-[var(--sh-border-soft)]">
                    ...
                    <td className="py-3 text-right text-[var(--sh-text-soft)]" style={{ color: agency.branding.primaryColor }}>{formatNumber(p.visitors)}</td>
                    <td className="py-3 text-right text-[var(--sh-muted)] font-mono text-xs">{p.avgTime}</td>
        ...
        <Card className="p-5">
          <h3 className="font-display text-[var(--sh-text)] text-lg mb-4">Actividad reciente</h3>
          <ul className="flex flex-col gap-1">
            {agency.activity.slice(0, 8).map(a => (
              <li key={a.id} className="flex items-start gap-3 py-2.5 border-b border-[var(--sh-border-soft)] last:border-0">
                <span
                ...
                <div className="min-w-0">
                  <p className="text-sm text-[var(--sh-text)] leading-snug">{a.description}</p>
                  <span className="text-xs text-[var(--sh-faint)] font-mono">{a.time}</span>
                </div>
              </li>
            ))}
          </ul>
