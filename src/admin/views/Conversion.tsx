  ...
  return (
    <div className="animate-fade-in">
      ...
      {tab === 'seguimiento' && (
        <div>
          <Card className="p-5 mb-5">
            <h3 className="font-display text-white text-lg mb-1">Recorrido comercial del visitante</h3>
            <p className="text-sm text-white/45">De visitante anónimo a oportunidad comercial calificada.</p>
            <div className="mt-5 flex flex-col md:flex-row items-stretch justify-between gap-2">
              {journey.map(([stage, desc], i) => (
                <div key={stage} className="flex-1 flex items-center gap-2">
                  ...
                  {i < journey.length - 1 && <span className="text-white/25 hidden md:block">→</span>}
                </div>
              ))}
            </div>
          </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="p-5">
              <h3 className="font-display text-white text-lg mb-4">Ejemplo de seguimiento real</h3>
              <div className="flex flex-col gap-2 font-mono text-xs">
                <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ background: agency.branding.primaryColor }} /><span className="text-white/75">10:32 — María Fernanda visitó Casa Laureles</span></div>
                <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ background: agency.branding.primaryColor }} /><span className="text-white/75">10:34 — Consultó a la IA por cochera</span></div>
                <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ background: agency.branding.primaryColor }} /><span className="text-white/75">10:40 — Solicitó información</span></div>
                <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ background: agency.branding.primaryColor }} /><span className="text-white/75">10:42 — Solicitó visita</span></div>
                <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ background: agency.branding.primaryColor }} /><span className="text-white font-semibold">Visita confirmada → Oportunidad</span></div>
              </div>
            ...
            <Card className="p-5">
              <h3 className="font-display text-white text-lg mb-4">Estado de oportunidades</h3>
      ...
      {tab === 'automatizaciones' && (
        <Card className="p-5">
          <h3 className="font-display text-white text-lg mb-1">Automatizaciones</h3>
          <p className="text-sm text-white/45 mb-4">Reglas que mantienen el seguimiento activo y a los asesores notificados.</p>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {agency.automations.map(a => (
              <div key={a.trigger} className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-sm text-white/90 font-medium">{a.trigger}</h4>
                  <Badge accent={a.status === 'activo' ? '#34d399' : '#fbbf24'}>
                    {a.status === 'activo' ? 'Activo' : 'Próximamente'}
                  </Badge>
                </div>
                <p className="text-xs text-white/50">→ {a.action}</p>
              </div>
            ))}
          </div>
      ...
      {tab === 'metricas' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="p-5">
            <h3 className="font-display text-white text-lg mb-4">Embudo de conversión</h3>
            <div className="flex flex-col gap-3">
              {agency.analytics.funnel.map(f => (
                <div key={f.label}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm text-white/75">{f.label}</span>
                    <span className="font-mono text-sm text-white">{formatNumber(f.value)}</span>
                  </div>
                  <div className="h-6 rounded-lg overflow-hidden border border-white/10 bg-white/5">
                    <div
          ...
          <Card className="p-5">
            <h3 className="font-display text-white text-lg mb-4">Conversiones</h3>
            <div className="grid grid-cols-2 gap-3">
              {agency.analytics.conversions.map(c => (
                <div key={c.label} className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-center">
                  <div className="font-display text-3xl text-white" style={{ color: agency.branding.primaryColor }}>{c.value}</div>
                  <div className="font-mono text-[10px] uppercase tracking-widest text-white/45 mt-1">{c.label}</div>
                </div>
              ))}
            </div>
            <p className="mt-4 text-xs text-white/40">
